...
# 調整資料形狀並標準化
X_train = X_train.reshape(-1, 28, 28, 1) / 255.0  # 新增通道維度 (28, 28, 1)
X_test = X_test.reshape(-1, 28, 28, 1) / 255.0

# 動態生成 TensorBoard 日誌目錄
log_dir = os.path.join("tb_logs_cnn", datetime.now().strftime("%Y%m%d-%H%M%S"))
...
model = Sequential([
    ...
    MaxPooling2D((2, 2)),  # 第一個池化層
    Conv2D(64, (3, 3), activation='relu'),  # 第二個卷積層
    MaxPooling2D((2, 2)),  # 第二個池化層
    Flatten(),  # 展平層
